...
# -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# N O T E S #
    # Run game loop somehow to progress the game via choice, input, draw, discard, play, etc.
    # run(fun_num = 1) (1 represents the_draw_1), (2 represents the_draw_2, etc.). run(fun_num) returns next_fun_num. the next_fun_num is the next function to be called AFTER draw_window is called. this is to be repeated endlessly to progress through the game so that a progression function is called, and then the screen is updated via draw_window's subsequent call. While each progression loop from run(fun_num) is executed, card locations are altered. draw_window()'s next call will call movements(), which will analyze each player's cards'locations and based on that update the coordinates, therefore the screen placement, of all cards properly.
    # How will next_fun_num be called through run(fun_num)? How about next_fun_num_var = run(fun_num) which returns next_fun_num, then

    # def progression_loop(fun_num):
        # new_fun_num = run(fun_num) > which returns new_fun_num (and also some values, variables that signify which cards were repositioned to be passed to draw_window() which in turn are passed to movements() so that every single card/card group does not need to be handled in order to properly update display?)
        # expansion on comment for above line - since run(fun_num) is dependent on each function having an associated number, could I just call the function directly instead of assigning each function a number?
        # draw_window() (which calls movements() (which will determine the screen location for each of 108 cards? or will it only access that which was altered through last progression_loop? this would require ))
        # return progression_loop(next_fun_num)

        # progression_loop(1)

    # PROBLEM WITH ABOVE THEORY - This methodology/structure seems to limit the 'responsiveness' of the game updating the screen. It's not going to update the screen locations until the entire function is done, and in many cases mid-function location alterations are made. Therefore I think I may have to make it so that draw_window() is called WITHIN the progression functions every time a location alteration is made

    # Every time a card is moved, it must be visually updated on the screen, which means every time a card is moved, the screen must be updated.

    # As to the point of when and how a card location is updated, how is this to be executed? will a card's coordinate be assigned whenever it is added to a particular group? for instance, if card is moved from hand to meld, is the location updated/assigned whenever the transfer is made to the meld, meaning that whenever an individual card's location/coordinate is altered, the group is rendered with the next location via draw_window's next call?

    # GREAT I D E A - Create an 'moved_cards' list attached to some other parent object (MasterDeck?) as an attribute that is updated after every progression_loop() is ran, and which is then accessed by movements() so that only the altered cards have their screen location updated. Movements() will then determine their respective locations, and run only the associated functions that update those locations.
    # GREAT I D E A EXPANDED - for card in moved_cards: > update location (VERY SIMPLE)
    # If card in P1.melds : move to Locations.p1_next_melds/cards_location (next_meld / next_card_location is to be determined by a registry of preexisting melds and cards in a group so that instead of having to re-iterate through every meld, only the altered melds and cards are handled.)
    # Whenever card is appended to a card group, add card to moved_cards AND the new_card_location (p1.meld[0]), etc. This could be accomplished by making moved_cards a dict, with the card as the key and the value as the new_card_location, or simply as an individual attribute such as Card.current_location (which is the new_card_location).
    # What format will the new_card_location be in? It has to be whatever most easily transfers to a format that accesses it's new location on the screen. This would imply it being something like Locate.p1.meld[0]
    # I think format should be the location function / function name associated with the location's placement assessment function. Aka the actual function / function name that location is going to call. If I can't pass it directly as an arg / value, then perhaps as the function name as a string, which would be handled after it is called by movements() via eval(Locate.'function_name'()) or exec(Locate.'function_name'())
    # I think I could just send it in as the actual function directly.
# -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#  T H I N G S  T O  D O  #
...
class Locations():
    def __init__(self):
        self.deck_location = []
        self.discard_pile_location = []
        self.p1_hand_location = []
        self.p2_hand_location = []
        self.p1_melds_location = [100, 800]
        self.p2_melds_location = []
        self.p1_red_3_location = []
        self.p2_red_3_location = []
        self.top_left_visible = [50, 70]
        self.center = [1010, 610]
        self.card_height_width = [100, 140]
        self.moved_cards = []
    # -------------------------------------
    ...
    def meld_location_situate(self, player): # It seemns best to make it so that these child functions within the parent function movements() take in a param that designates "new members", "removed members", etc. Could this just be the card objects?
        meld_num = 0
        # Below Line - Changes meld's x-coordinate. For separation of each meld. Moves by card width + 20 (for empty space between).
        meld_increase_x = self.card_height_width[0] + 20
...
# -------------------------------------
# Below Function - Called by draw_window(). Contains all function calls from Locate to handle card position assignments based on logic flow.
def movements(): # I think I need to make it so that a param is passed into this function each time draw_window() is called which represents which function to call from the array of movement functions located within this parent function. For instance, progression_loop() returns a num (the param to be passed here, dependent upon which card group was altered).
    card_num = 0
    for card in Locate.moved_cards.keys():
        # relocate cards
        Locate.moved_cards.values()[card_num]
        card_num += 1
        # Change meld_location_situate to detect the location of ONLY the new cards, saving the locations of the pre-existing cards (for faster assessment/runtime)
        Locate.meld_location_situate(P1)
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
