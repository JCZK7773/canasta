...
def the_draw_2(testing = False):
    ...
    # -------------------------------------
    for current_player in [player.P1, player.P2]:
        game.game.progression_text = (f"{current_player.name}, Select your card from the stack to determine which player will have the first play. Click a card to choose.")
        ...
        while True: # ****
            ...
            # -------------------------------------
            temp_meld = []
            temp_meld.append(deck.MasterDeck.deck.pop(deck.MasterDeck.deck.index(current_player.draw_card)))
            # -------------------------------------
            game.game.clicked_card = None
            game.game.clicked_card_list.clear()
            # -------------------------------------
            # Below Section - Checks to see if the current_player's draw_card is a Joker; if so, places the card back into the MasterDeck from the current_player.hand, and assigns the .draw_card to None. It will then make the current_player choose another card. If not, calls the_draw_3().
...
def the_draw_3(testing = False): # ****
    ...
    if (player.P1.draw_card.rank, player.P1.draw_card.suit) == (player.P2.draw_card.rank, player.P2.draw_card.suit): # ****
        ...
        if deck.MasterDeck.draw_suit_ranks.get(player.P1.draw_card.suit) > deck.MasterDeck.draw_suit_ranks.get(player.P2.draw_card.suit): # ****
            ...
            return the_deal(player.P2, player.P1) # ****
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# Below Function - Called by the_draw_3() function. Returns the player.draw_card from player.hand back into MasterDeck.deck. # ***
...
def sorter_key_function(item): # ****
    ...
    # Below - If item is a meld (list). # ****
        ...
        if item[0].rank != 'Joker': # ****
            ...
            return int(1) # ****
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
###### Below Function - Not sure I will continue to use this in the 2D version of the game. Called by play_1(), draw_discard_pile_attempt_temp_meld_wild_card_addition(), play_2(), valid_play_check_and_sort(), wild_card_meld_choice_prompt(), discard(), went_out_check() functions. Miscellaneous function for handling printed lists that I want to be sorted and then numbered; for the purpose of input choice selection via the preceding num. # ****
...
def red_3_check(current_player, drawn = False): # ****
    ...
    for current_card in current_player.hand[:]: # ****
        if (current_card.rank, current_card.suit) in deck.Deck().red_3s: # ****
            if drawn == True: # ****
                game.game.progression_text = (f"{current_player.name}, you drew a Red 3! The {card} will be placed in your Red 3 meld, and you will automatically draw another card to replace it.") # ****
                game.game.xs_display(2)
                current_player.red_3_meld.append(current_player.hand.pop(-1)) # ****
                current_player.hand.append(deck.MasterDeck.deck.pop(0)) # ****
                ###### Below Line - I think I want to disable this. Not necessary.
                # game.game.progression_text = (f"In place of your Red 3, you drew a: {current_player.hand[-1]}") # ****
            # Below Section - Handles initial Red 3 check. Appends red_3_meld to player.melds, replaces it with a new card from MasterDeck.deck, & prints out associated output for initial Red 3 distribution. # ****
            elif drawn == False: # ****
    ...
    if len(current_player.red_3_meld) == 4: # ****
        ...
        game.game.progression_text = (f"{current_player.name}, you have {len(current_player.red_3_meld)} Red 3(s) in your hand! The rules dictate that all Red 3s must be played down, and you have to withdraw a replacement card from the stock for each one! {current_player.name}, in place of your Red 3(s), you drew a " + ', '.join(['%s']*len(current_player.red_3_meld)) % tuple(current_player.hand[-(len(current_player.red_3_meld)):])) # ****
        game.game.xs_display(3)
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
def play_1(current_player): # ****
    ...
    logger.debug("play_1\n") # ****
    # -------------------------------------
    game.game.progression_text = (f"{current_player.name}, which method would you like to use to draw?")
...
def stock_draw(current_player, from_draw_discard_pile_attempt_no_canasta_fail = False): # ****
    ...
    if len(MasterDeck.deck) > 0: # ****
        game.game.progression_text = (f"{current_player.name} drew a {deck.MasterDeck.deck[0]} from the stack.")
        current_player.hand.append(deck.MasterDeck.deck.pop(0)) # ****
        red_3_check(current_player) # ****
...
def valid_play_check_and_sort(current_player): # ****
    ...
    for current_card in current_player.play_cards[:]: # ****
        ...
        if card_found_in_meld == False: # ****
            ...
            if (current_card.rank, current_card.suit) in Deck().black_3s: # ****
                if len(current_player.hand) <= 1: # ****
            ...
            elif (current_card.rank, current_card.suit) in Deck().wild_cards: # ****
...
def wild_card_meld_choice_prompt(current_player, wild_card): # ****
    ...
    if len(current_player.play_cards) > 0:
        for meld in current_player.play_cards:
            if game.game.clicked_card in meld:
                print('game.game.clicked_card in play_cards')
                meld_choice_index = game.game.clicked_card
    if len(current_player.melds) > 0:
        for meld in current_player.melds:
            if game.game.clicked_card in meld:
                print('game.game.clicked_card in melds')
                meld_choice_index = game.game.clicked_card
...
def discard(current_player): # ****
    ...
    MasterDeck.discard_pile.append(current_player.hand.pop(current_player.hand.index(game.game.clicked_card))) # ****
    # -------------------------------------
...
def went_out_check(current_player, going_out_from_discard_draw = False): # ****
    ...
    if current_player.has_canasta == True: # ****
        ...
        if current_player == P1: # ****
            P2.going_out = False # ****
            game.game.progression_text = f'{P1.name}, you successfully went out!'
            game.game.xs_display(2)
            game.game.progression_text = f'Round Score: {P1.name}: {P1.round_score} {P2.name}: {P2.round_score}' # ****
            game.game.xs_display(3)
        else: # ****
            P1.going_out = False # ****
            game.game.progression_text = f'{P2.name}, you successfully went out!'
            game.game.xs_display(2)
            game.game.progression_text = f'Round Score: {P2.name}: {P2.round_score} {P1.name}: {P1.round_score}' # ****
            game.game.xs_display(3)
            # -------------------------------------
        round_reset() # ****
        # -------------------------------------
...
def round_reset(): # ****
    ...
    # --------------------------------------
    game.game.progression_text = (f"The round is over! {P1.name}'s final round score was {P1.finished_rounds_scores[-1]}, and {P2.name}'s final round score was {P2.finished_rounds_scores[-1]}.")
    game.game.xs_display(3)
    if P1.finished_rounds_scores[-1] > P2.finished_rounds_scores[-1]:
        game.game.progression_text = (f"{P1.name} was the winner of the round!")
        game.game.xs_display(1)
        ...
        game.game.progression_text = (f"{P2.name} was the winner of the round!")
        game.game.xs_display(1)
    # --------------------------------------
    deck.MasterDeck.discard_pile.clear()
    deck.MasterDeck.deck.clear()
    deck.MasterDeck.deck = MasterDeck.original_deck[:]
    # --------------------------------------
    win_check() # ****
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
