######## THIS IS OUTDATED ####### CHROMEBOOK HAS MOST UP TO DATE VERSION ########
# Things to do
# 1) Change prints for debugging into logs for backend debugging so as not to muddy the MUD display during gameplay?
# 2) Write and run some thorough tests for the program to go through for finding and fixing hidden bugs.
# 3) Go through the entire script to remove all extra comments and test functions, etc.
# 4) Post on web so others can check for bugs as well.
# Stopped doing the final check @ line 553.
# -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
import random # ****
import sys # ****
# -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
class Player(): # ****
    ...
    def round_score(self): # ****
        round_score = 0 # ****
        # -------------------------------------
        for meld_group in [self.play_cards, self.melds, self.black_3_meld]: # ****
            if len(item) > 0:
                for item in meld_group: # ****
...
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# Below - Creates the MasterDeck & Deck2, combines the two together, and shuffles them, with MasterDeck becoming the Deck for the game. # ****
MasterDeck = Deck() # ****
Deck2 = Deck() # ****
# -------------------------------------
...
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# Below - Creates the players, and gives them placeholder names for identification at start of game. # ****
P1 = Player('Player 1') # ****
P2 = Player('Player 2') # ****
players = [P1, P2] # ****
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# Below Function - First Function for Game Loop. Handles logic for sequencing of the_draw functions for when certain criteria require certain sections to be rerun. # ****
...
def the_draw_2(player): # ****
    # Follow Up to the_draw_1 for Game Loop. Handles name creation, draw card choice (to determine first player), and ensures that if a Joker is drawn during this process, it is reset and redone (Logic for that handled in draw_joker_check). # ****
    print("draw 2") # ****
...
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
def the_draw_3(): # ****
    # Follow Up to the_draw_1() for Game Loop. Checks whether or not the players have the same draw card, and if they do, redirects the process back to the_draw_1 and places player cards back into the MasterDeck. Also checks to see who wins the draw, and redirects to the_deal() after determination. # ****
    print("draw 3\n") # ****
    # ------------------------------------- # ****
...
def return_draw_card(players): # ****
    # Stems from the_draw_3. Simple function to return the draw card used to determine first play from player.hand back into MasterDeck.deck(). For purpose of reducing amount of repeated code. # ***
    print("return_draw_card\n") # ****
    for player in players: # ****
        MasterDeck.deck.append(player.hand.pop(0)) # ****
# -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# Below Function - Follow up to the_draw_3() for Game Loop. Handles dealing, sorting, printing; red 3 checks each player's cards, creates a discard pile from MasterDeck, and directs to play_1(). # ****
...
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# Below Function - Follow up to the_deal() for Game Loop. Gives the player the option to either draw from the deck or to attempt a draw from the discard pile. Also prints out all preexisting melds for reference as it is important for choosing between the draw options. # ****
def play_1(player): # ****
    print("play_1\n") # ****
    # -------------------------------------
    while True: # ****
        ...
        # Below Section - If the meld consists of only 1 natural card from the hand and the face up discard; if the discard_pile_is_frozen == True, reroutes the player to stock_draw(). # ****
            ...
            if MasterDeck.discard_pile_is_frozen == True: # ****
                ...
                if player.hand_wild_cards_reference_list > 0: # ****
                    ...
                    stock_draw(player) # ****
                # -------------------------------------
    # Below - If the player did not have any matching cards to the face up discard in their hand. # ****
    else: # ****
...
def replace_discard_pile_temp_meld(player): # ****
    # Below - Replaces the face up discard back into the discard pile and replaces the matched cards from the player.meld back into the player.hand. # ****
    MasterDeck.discard_pile.append(player.melds[-1].pop(-1)) # ****
    for matched_card in matched_card_list: # ****
        player.hand.append(player.melds[-1].pop(-1)) # ****
    player.melds.pop(-1) # ****
...
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# Below Function - Stems from play_1() function.  # ***
def play_2(player): # ***
    print("play_2\n") # ***
    # -------------------------------------
    ...
    # -------------------------------------
    if play_choice == "1": # ***
        print(f"\n{player.name}, which cards would you like to play? (Use the ordered number of the card(s) as listed below for choosing the associated card(s) for play. (i.e. - '1, 3, 4, 8')\n") # ***
        sorted_and_numbered_list_printer(player.hand) # ***
...
# -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# Below Function - Stems from play_2() function. For ensuring that a set of attempted play cards are valid according to game rules. # ***
