...
def draw_discard_pile_attempt(player, stock_depleted = False): # ****
    ...
    # -------------------------------------
            ...
            # -------------------------------------
    # Below Section - In the case that a player attempts to draw the discard pile whenever the face up discard is a Wild Card or a Black 3. Outputs an informative prompt and returns stock_draw().
    ...
    # Below Section - Since the player was unable to draw the discard pile after attempting all of the possible methods, they must instead draw from the stock; calls stock_draw(player).
    print("It looks as if you were unable to draw the discard pile after attempting all of the possible methods. You will draw from the stock instead.\n")
    stock_draw(player)
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
def draw_discard_pile_attempt_check_hand_match(player): # ****
    ...
    if len(player.matched_card_list) > 0: # ****
        ...
        # -------------------------------------
        # Below Section - If the player has 2 natural matches for the face up discard, (plus the face_up_discard in player.melds[-1]). Checks that the player has met meld requirement. Also checks to see if the player is going out. If the player is validly going out, or is not attempting to go out, draws discard pile successfully. If the player is not validly going out, the drawing of the discard pile is cancelled and the matching cards and the face up discard are replaced back to their original location and the player is redirected to draw from the stock. If the player does not reach the meld_requirement, attempts to use additional wild cards within the hand to reach the meld requirement via draw_discard_pile_wild_card_prompt. # ****
        if len(player.melds[-1]) >= 3: # ****
            if player.round_score >= player.meld_requirement: # ****
                if len(player.hand) < 1:
                    if went_out_check(player, from_hand_check_discard_pile = True) == False:
                        MasterDeck.discard_pile.append(player.melds[-1].pop(-1))
                        for card in player.melds[-1][:]:
                            player.hand.append(player.melds[-1].pop(-1))
                        player.matched_card_list.clear()
                        return stock_draw(player)
                    else:
                        print("You successfully created a valid meld from the face up discard!\n")
                        return draw_discard_pile(player, from_hand_check_discard_pile = True) # ****
...
def draw_discard_pile(player, from_hand_check_discard_pile = False): # ****
    ...
    if from_hand_check_discard_pile == False:
        red_3_check(player) # ****
        play_2(player) # ****
    else:
...
def valid_play_check_and_sort(player): # ****
    ...
    for temp_meld in player.play_cards[:]: # ****
        ...
        # -------------------------------------
    # Below Section - Sorts both player.initial_played_cards & player.melds so that whenever the two are subsequently compared and both have the same exact cards, they will be equal to one another because everything is of the same order, instead of being != although the contents are equal.
    player.initial_played_cards.sort(key=sorter_key_function)
    player.melds.sort(key=sorter_key_function)
    # -------------------------------------
...
def went_out_check(player, from_hand_check_discard_pile = False): # ****
    ...
    # Below Extended Section - In the case that a player is attempting to go out (has 0 cards in their hand) but does not have the required canasta to do so. Gives the player an informative message and prompts them to choose 2 cards to take back into their hand from the available melds to use as a discard and a required card to be kept in their hand. # ****
        ...
        if from_hand_check_discard_pile == False:
            ...
            print("It looks as if you are attempting to go out, but you do not have a Canasta, which is required. You must cancel this draw from the discard pile because if you did not, you would be left with 0 cards in your hand, but would be unable to go out. This is against the rules. You must instead draw from the stock.\n")
            return False
    # -------------------------------------
    if player == P1: # ****
        return play_1(P2) # ****
