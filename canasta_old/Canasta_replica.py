...
# ------------------------------
# Below Section - I THINK I FIXED THIS! In the below snippet, a card was added to a preexisting meld of Kings, but after the play it said that the player was unable to succeed in playing ony of their attempted melds.

# Your existing melds are as follows:
#
# 1) [9♣, 9♠, 2♥]
# 2) [K♣, K♦, K♦]
#
# j2, what would you like to do with your turn?
#
#  1) Play Cards
#  2) Discard
#
# > 1
#
# j2, which cards would you like to play? (Use the ordered number of the card(s) as listed below for choosing associated card(s) for play.)
#
# 1) 8♣
# 2) 8♥
# 3) 10♥
...
# > 1,2,3,4,5,6
#
# j2, you have some attemped melds that do not pass the rule requirements. You cannot create a new meld without at least 2 natural cards of the same rank, or without a wild card to add to it to reach the 3 card minimum meld requirement. The attempted meld(s) will be removed and placed back in your hand:
#
# 1) [8♥, 8♣]
# 2) [10♠, 10♥]
# 3) [J♦]
#
# It looks as if you were unable to succeed in playing any of your attempted play cards. Therefore you will have to choose another set of play cards, or opt to discard instead if you have no valid plays to make.
# -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# Things to do
# Make sure player.matched_card_list is not being used, but instead that a copy is being used in certain cases for dealing with drawing the discard pile functions.
# Add in reset_deck() function for MasterDeck.deck in win_check() function. Also check to make sure everything that needs to reset is done so.
# Do I actually need non_num_card_rank_sort_value? I think I could just use MasterDeck.draw_ranks
# Remove Deck2 and make it so that MasterDeck creates a double deck.
# 1) Lines 474-..., 488-489 correct? Can I iterate through a different list to use the length of it as a reference for what to .pop and append from 2 other lists?? Didn't want to work in Skulpt... but I don't think Skulpt works...
# 2) Make sure that all places I am doing .pop on all items in a list, that I am using [:] beforehand so that all elements are actually being removed, as trying this without doing that first causes elements to be skipped over, even when using .pop(-1).
# 3) Write and run some thorough tests for the program to go through for finding and fixing hidden bugs.
...
logging.basicConfig(filename = "J:\\Programming\\Projects\\Canasta\\canasta_replica\\Canasta_log.log", level = logging.DEBUG, format = LOG_FORMAT, filemode = 'a') # ****
...
# -------------------------------------
# Below Line - Shuffles the MasterDeck.
...
def draw_discard_pile_attempt_check_meld_match(player, stock_depleted = False): # ****
    ...
    if stock_depleted == True: # ****
        ...
        round_reset() # ****
        # -------------------------------------
    print(f"\n{player.name}, unfortunately you were unable to draw the discard pile because you did not have a matching meld. You will instead draw from the stock.\n") # ****
    stock_draw(player) # ****
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
def draw_discard_pile_attempt_check_hand_match(player): # ****
    ...
    if len(player.matched_card_list) > 0: # ****
        # Below Section - Copies the player.matched_card_list so as to create a fixed, static rank value, as the variable player_matched_card_list_copy. Next the player_matched_card_list_copy is appended to player.melds, and finally appends the face_up_discard to that meld, popping the cards from their original locations. # ****
    ...
    # Below Section - If the player did not have any matching cards to the face up discard in their hand; redirects player to stock_draw. # ****
        ...
        print("Sorry, but you did not have any matches to the face up discard in your hand. You will instead have to draw from the stock.\n") # ****
        stock_draw(player) # ****
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
def draw_discard_pile_attempt_temp_meld_wild_card_addition(player): # ****
    ...
    print(f"{player.name}, which wild card would you like to use? You have these wild cards:\n") # ****
...
def draw_discard_pile_wild_card_prompt(player):
    ...
    while len(player.hand_wild_cards_reference_list) > 0: # ****
        while True: # ****
            try: # ****
                use_wild_card_input = input(f"Unfortunately your attempted meld did not meet the meld requirement, therefore you would have to add a/some wild card(s) to the meld to see if that would help you to reach the score requirement. It looks as if you have {len(player.hand_wild_cards_reference_list)} wild card(s) to use for this. Would you like to do this? (Y/N)\n\n> ") # ****
                if use_wild_card_input.lower() not in ('y', 'n'): # ****
                    raise ValueError # ****
                break # ****
            except ValueError: # ****
                print("Sorry, but there was a problem with your input. Please try again, making sure that your input is either 'Y' or 'N' (Yes or No).\n") # ****
                # -------------------------------------
        if use_wild_card_input.lower() == 'y': # ****
            ...
            # -------------------------------------
    print("Unfortunately your attempted meld did not meet the meld requirement, and you do not have any wild cards to add to it to help reach the required score. Your attempted meld will be placed back in your hand, and the face up discard will be placed back into the discard pile. You will instead draw from the stock pile.\n") # ****
    return replace_discard_pile_temp_meld(player) # ****
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
def valid_play_check_and_sort(player): # ****
    ...
    player.initial_played_cards = player.melds[:] # ****
    # -------------------------------------
    ...
    for card in player.play_cards[:]: # ****
        ...
        if card_found_in_meld == False: # ****
            # Below Section - Checks if card is a Black 3 and whether the player meets the card count requirements to be able to play a Black 3 meld. If not, appends it to play_cards_removed_black_3s_print, which is used to print out all of these instances later on to inform the player, and removes it from player.play_cards, placing it back into player.hand. If so, appends the Black 3s into player.black_3_meld. # ****
            if (card.rank, card.suit) in Deck().black_3s: # ****
    ...
    for temp_meld in player.play_cards[:]: # ****
        ...
        # -------------------------------------
    if player.initial_played_cards == player.melds: # ****
        ...
        logger.debug("initial_played_cards != melds\n") # ****
        # -------------------------------------
        if len(player.hand) <= 1: # ****
    ...
    # ------------------------------------- # ****
            # Below Section - If the amount of wild cards being played is greater than 7; player is prompted to choose which wild cards they want to use in the canasta. The non-chosen wild cards will remain in play to be distributed later in the function, unless this is the player's first meld. In this case, appends left over wild cards back into player.hand, and returns None. # ****
                ...
                if player.round_score < player.meld_requirement: # ****
                    ...
                    return None # ****
            # -------------------------------------
    # Below Section - Runs if the player has less than 7 wild cards in player.play_cards_wild_cards OR if they have 7 or more wild cards but chose not to create a wild card canasta with them, and if the player has at least 1 meld in player.play_cards or player.melds that do not contain the max amount of wild cards to add the wild cards to; iterates through each wild card in player.play_cards_wild_cards and prompts the player to choose a destination for each card. # ****
...
def went_out_check(player): # ****
    ...
    if player.has_canasta == True: # ****
        ...
        round_reset() # ****
        # -------------------------------------
    # Below Section - In the case that a player is attempting to go out (has 0 cards in their hand) but does not have the required canasta to do so. Gives the player an informative message and prompts them to choose 2 cards to take back into their hand from the available melds to use as a discard and a required hand card. # ****
    else: # ****
        ...
        for meld_index in range(len(player.melds)): # ****
            ...
            if player.initial_played_cards[meld_index]: # ****
            ...
            # Below Line - For melds that were created during current play. # ****
                ...
                for card in player.melds[meld_index]: # ****
                    player.last_set_played_cards.append(card, "-", player.melds[meld_index]) # ****
        # -------------------------------------
        # Below Section - Prints all of the last played cards and their associated melds for reference when choosing a card to be replaced back into the hand. # ****
        print("The cards that were used last play are listed below, followed by their associated melds for reference in helping to determine which card to take back into your hand. In the case that a card from the affected/created meld consists of less than 4 cards, the card's meld you choose will be completely disbanded and replaced back into your hand. If it is found that after card or meld is replaced back into your hand you no longer meet the meld requirement, all of your played cards will be placed back into your hand and you will have to make another play attempt. You have to choose 2 cards to be placed back (1 for keeping in the hand and 1 for discarding).\n") # ****
        sorted_and_numbered_list_printer(last_set_played_cards) # ****
        # -------------------------------------
        # Below Section - Runs the went_out_check_replacement_card() function twice because the player has to choose 2 cards to be replaced back into the hand. # ****
...
def went_out_check_replacement_card(player): # ****
    ...
    while True: # ****
        try: # ****
            ...
            if len(player.melds[player.melds.index(last_set_played_cards[replacement_choice - 1][2])]) > 3: # ****
                print(f"You removed the {last_set_played_cards[replacement_choice - 1][0]} from it's meld back into your hand.\n") # ****
                # Below Line - Appends the chosen card to the player.hand, popping from player.meld. # ****
                player.hand.append(player.melds[player.melds.index(last_set_played_cards[replacement_choice - 1][2])].pop(player.melds[player.melds.index(last_set_played_cards[replacement_choice - 1][2])].index(last_set_played_cards[replacement_choice - 1][0]))) # ****
            else: # ****
                ...
                for meld in player.last_set_played_cards: # ****
                    if len(meld) > 3: # ****
                        melds_len_over_3 = True # ****
                if melds_len_over_3 == True: # ****
                    while True: # ****
                        try: # ****
                            meld_verify_input = input("This meld has 3 cards, which means the entire meld would have to be disbanded if you were to remove this card. It looks like you have a meld in your list of choices that has more than 3 cards, which means it would not have to be completely disbanded. Are you sure that you want to disband this meld instead? (Y/N)\n\n> ") # ****
                    ...
                    if meld_verify_input.lower() == 'y': # ****
                        ...
                        for card in player.melds[player.melds.index(last_set_played_cards[replacement_choice - 1][2])]: # ****
                            player.hand.append(player.melds[player.melds.index(last_set_played_cards[replacement_choice - 1][2])].pop(-1)) # ****
                    ...
                    print("The meld associated with this card consists of 3 cards, so the entire meld will be disbanded and placed back into your hand, as that is the least amount of cards allowed in a valid meld.\n") # ****
                    for card in player.melds[player.melds.index(last_set_played_cards[replacement_choice - 1][2])]: # ****
                        player.hand.append(player.melds[player.melds.index(last_set_played_cards[replacement_choice - 1][2])].pop(-1)) # ****
                    # Below Line - Breaks out of the while loop to reroute the player back to make a new card choice. # ****
                    break # ****
                # -------------------------------------
        except (ValueError, IndexError): # ****
            print("Sorry, but there was a problem with your input. Try again, making sure that your input is the number that precedes the card you wish to choose.\n") # ****
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# Below Function - Called by draw_discard_pile_attempt(), draw_discard_pile_attempt_check_meld_match(), went_out_check() functions. Resets all settable attributes and values back to default/starting conditions to start a new round. # ****
def round_reset(): # ****
    ...
    win_check() # ****
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# Below Function - Called by round_reset() function. Checks a winning value against each player's score to determine whether or not someone has won. If so, calls winner_output() function. If not, begins a new round via the_draw_1() function. # ****
def win_check(): # ****
    ...
    for player in players: # ****
        if player.total_score >= 5000: # ****
            if player == P1: # ****
                if player.total_score > P2.total_score: # ****
                    return winner_output(P1) # ****
                elif player.total_score == P2.total_score: # ****
                    winner_output(player, True) # ****
                elif player.total_score < P2.total_score: # ****
                    return winner_output(P2) # ****
            else: # ****
                if player.total_score > P1.total_score: # ****
                    return winner_output(P2) # ****
                elif player.total_score == P1.total_score: # ****
                    winner_output(player, True) # ****
                elif player.total_score < P1.total_score: # ****
                    return winner_output(P1) # ****
     # -------------------------------------
