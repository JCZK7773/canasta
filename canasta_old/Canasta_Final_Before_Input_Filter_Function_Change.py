...
# -------------------------------------
# Finish play_again_input and other inputs so that they will only accept a yes or no answer via a while loop.
# Make the try, except code for inputs into a function since I am reusing it in various places.
# Finish going through all of the code to ensure that exception handling is complete.
# -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
import random # ***
import sys # ***
# -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
def play_1(player):
    ...
    while True:
        try:
            ...
            if draw_method != '1' or draw_method != '2':
                raise ValueError
            break
        except ValueError:
            print("Sorry, but there was a problem with your input. Please try again. Make sure to use only either '1' or '2' as your input.\n")
            ...
            if stock_depleted == True:
                while True:
                    try:
                        ...
                        if draw_rest_of_discard_pile_input.lower() != 'y' or draw_rest_of_discard_pile_input.lower() != 'n':
                            raise ValueError
                        break
                    except ValueError:
                ...
                if len(player.hand_wild_cards_reference_list) > 0: # ****
                    while True:
                        try:
                            ...
                            if use_wild_card_for_discard_pile.lower() != 'y' or use_wild_card_for_discard_pile.lower() != 'n':
                                raise ValueError
                            break
                        except ValueError:
                            print("Sorry, but there was a problem with your input. Please try again. Make sure that your input is either 'Y' or 'N' (Yes or No).\n")
                            ...
                            if len(player.hand_wild_cards_reference_list) > 0:
                                while True:
                                    try:
                                        ...
                                        if add_more_wild_cards.lower() != 'y' or add_more_wild_cards.lower() != 'n':
                                            raise ValueError
                                        break
                                    except ValueError:
                                        print("Sorry, but there was a problem with your input. Please try again. Make sure that your input is either 'Y' or 'N' (Yes or No).\n")
...
def draw_discard_pile_attempt_temp_meld_wild_card_addition(player):
    ...
    while True:
        try:
            ...
            print("Sorry, but there was a problem with your input. Please try again. Make sure that your input is one of the numbers preceding the card you would like to use.\n")
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
def play_2(player): # ***
    ...
    while True:
        try:
            ...
            if play_choice != '1' or play_choice != '2':
                raise ValueError
            break
        except ValueError:
            print("Sorry, but there was a problem with your input. Please try again. Make sure that your input is either '1' or '2' (Play Cards or Discard).\n")
    # -------------------------------------
    if play_choice == "1": # ***
        ...
        if len(player.non_maxed_out_melds) > 0: # ****
            for wild_card in player.play_cards_wild_cards[:]: # ****
                ...
                # Below - If player chooses to place the wild card into a meld; determines which meld group (player.play_cards or player.melds) they chose, and appends the wild card to that meld, popping it from play_cards_wild_cards. # ****
                    ...
                    print(f"{player.name}, you successfully added the {player.play_cards_wild_cards[player.play_cards_wild_cards.index(wild_card)]} to your meld.\n") # ****
                    player.melds[meld_choice_index].append(player.play_cards_wild_cards.pop(player.play_cards_wild_cards.index(wild_card))) # ****
                    # ------------------------------------- # ****
        else: # ****
...
def discard(player):
    ...
    while error_thrown == True:
        try:
            ...
            print("Sorry, but there was a problem with your input. Please try again.\n")
