...
# -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# Things to do
# Found a bug where I am using .clear() to clear lists after appending them to another place. This is BAD, because since the object that gets cleared is the one being referenced where it was appended, it is also cleared in that space as well. I should be using some_list[:] to create a reference to the sliced (thus newly created, object) instead. I need to go through and find places where I'm doing this and fix these instances.
# Is player.last_set_played_cards cleared whenever went_out_check_replacement_card is ran?
...
def the_draw_2(player, testing = False): # ****
    ...
    if player.name == 'Player 1' or player.name == 'Player 2': # ****
        while True: # ****
            try: # ****
                player.name = input(f"{player.name}, what is your name? \n\n> ") # ****
...
def valid_play_check_and_sort(player): # ****
    ...
    if len(player.black_3_meld) >= 3:
        ...
        print(f"Sorry, {player.name}, but your attempted Black 3 meld (made possible because you played 3 or more of them with 1 or less cards left in your hand) did not succeed because you must have at least 1 Canasta to be eligible for this play. Therefore, your Black 3s will be placed back into your hand.\n")
...
def went_out_check(player): # ****
    ...
    # Below Extended Section - In the case that a player is attempting to go out (has 0 cards in their hand) but does not have the required canasta to do so. Gives the player an informative message and prompts them to choose 2 cards to take back into their hand from the available melds to use as a discard and a required card to be kept in their hand. # ****
        ...
        if prior_hand_len == (len(player.hand) - 1):
            went_out_check_replacement_card(player) # ****
        # -------------------------------------
