...
# ------------------------------
# Below Section - Something is wrong with the code in wild_card_meld_choice_prompt() causing the input value to not be properly processed as the chosen card.
# J1, which cards would you like to play? (Use the ordered number of the card(s) as listed below for choosing associated card(s) for play.)
#
# 1) 2♣
# 2) 7♥
# 3) 7♦
...
#
# > 1
# J1, which meld from the list below would you like to add the 2♣ to?
#
# 1) [Q♣, Q♥, Q♣]
# 2) [A♥, A♥, 2♠]
#
# 3) I would not like to use this wild card, and would rather have it be replaced back into my hand.
#
# > 2
#
...
Deck2.create_deck() # ****
# -------------------------------------
# Below Section - Appends Deck2 to the MasterDeck and deletes Deck2.
...
def stock_draw(player): # ****
    ...
    if len(MasterDeck.deck) > 0: # ****
        player.hand.append(MasterDeck.deck.pop(0)) # ****
        print(f"\n{player.name} drew a {player.hand[-1]} from the stack.\n") # ****
        red_3_check(player) # ****
        play_2(player) # ****
    else: # ****
...
def draw_discard_pile_attempt_check_meld_match(player, stock_depleted = False): # ****
        ...
        if MasterDeck.face_up_discard.suit == meld[0].rank: # ****
            ...
            if stock_depleted == True: # ****
                ...
                if draw_rest_of_discard_pile_input.lower() == 'n': # ****
                    ...
                    # -------------------------------------
            # If draw_rest_of_discard_pile_input was 'Y'. # ****
    ...
    if stock_depleted == True: # ****
        ...
        round_reset() # ****
        # -------------------------------------
# Below - When stock_depleted == False and the player cannot draw the pile because of the above section not detecting a match for the face up discard. # ****
...
def draw_discard_pile_attempt_check_hand_match(player): # ****
    ...
    if len(matched_card_list) > 0: # ****
        # Below - Appends the matched_card_list to player.melds and then appends the face_up_discard to that meld, popping them from their original locations. # ****
        player.melds.append(matched_card_list) # ****
        player.melds[-1].append(MasterDeck.discard_pile.pop(-1)) # ****
        # Below - If the player has 2 natural matches for the face up discard. Checks that the player has met meld requirement. If so, draws discard pile successfully. # ****
        if len(matched_card_list) >= 2: # ****
            if player.round_score > player.meld_requirement: # ****
                ...
                # -------------------------------------
        # Below Section - If the meld consists of only 1 natural card from the hand and the face up discard; if the discard_pile_is_frozen == True, reroutes the player to stock_draw(). # ****
            ...
            if MasterDeck.discard_pile_is_frozen == True: # ****
                ...
                if player.hand_wild_cards_reference_list > 0: # ****
                    draw_discard_pile_wild_card_prompt(player) # ****
                else: # ****
                    ...
                    stock_draw(player) # ****
                # -------------------------------------
    # Below Section - If the player did not have any matching cards to the face up discard in their hand. # ****
    else: # ****
...
def draw_discard_pile_attempt_temp_meld_wild_card_addition(player): # ****
    ...
    while True: # ****
        try: # ****
            ...
            player.melds[-1].append(player.hand.pop(player.hand.index(player.hand_wild_cards_reference_list[int(wild_card_choice - 1)]))) # ****
            break # ****
        except (ValueError, IndexError): # ****
...
def valid_play_check_and_sort(player): # ****
    ...
    for meld in player.melds:
        player.initial_played_cards.append(meld) # ****
    # -------------------------------------
